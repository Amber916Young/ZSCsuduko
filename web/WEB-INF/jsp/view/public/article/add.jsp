<template:user_backend htmlTitle="发表文章" bodyTitle="发表文章">
    ...
    <div class="panel-heading">
        <h3 class="panel-title"><i class="glyphicon glyphicon-th"></i>发表文章</h3>
    </div>
    <div class="panel-body">
        <form class="form-inline" role="form" style="float:left;">
            <div class="form-group has-feedback" style="height: 110px;">
                <div class="input-group">
                    <div class="input-group-addon">请输入文章标题</div>
                    <input id="TitleText" class="form-control has-success" type="text" placeholder="请输入文章标题">
                </div>
                <br>  <br>  <br>
                <div class="input-group">
                    <div class="input-group-addon">请输入文章类型</div>
                    <select style=" " onclick="showtext(this)"  id="type" name="type" class="form-control has-success">
                ...
                <div class="input-group" id="msg">
                    <%--<div class="input-group-addon">请输入具体种类</div>--%>
                    <%--<input id="typemsg" class="form-control has-success" type="text" placeholder="例如：杀手数独">--%>
                </div>
                <br><br><br>
...
<script type="text/javascript">
    ...
    function upload() {
        ...
        if (type==""){
            layer.msg("数独种类不能为空，请输入", {time:2000, icon:5, shift:6}, function(){
            });
            return;
        }
        var arr = [];
        ...
        flag["typemsg"] = typemsg;
        flag["type"] = type;
        var json = JSON.stringify(flag);
        $.ajax({
            async: false,
            method:"POST",
            url:"${cx}/public/article/upload",
            data:json,
    ...
    function showtext(e) {
        var type  =e.value;
        if(type=="变形六宫数独"||type=="变形九宫数独"){
            $("#msg").html('<div class="input-group-addon">请输入具体种类</div>\n' +
                '                    <input id="typemsg" class="form-control has-success" type="text" placeholder="例如：杀手数独">');
        }else {
            $("#msg").html('');
        }
